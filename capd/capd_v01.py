...
class Package(object):		#object is there to make it a new-style class
	...
	def pkg_xml(self):
		return Software_Repo+"/apps/"+self.prefix+self.name+".xml"

	def pol_xml(self):
		return Software_Repo+"/apps/"+self.prefix+self.name+"_pol.xml"
...
def create_conf():
	...
	jss_share = raw_input("Casper share name (ex. CasperShare): ")
...
def add_pkg_prefix(pkg_prefix):
	...
	for pkg in pkgs:
		if not pkg.startswith('.') and not pkg.startswith(pkg_prefix):
			os.rename(Software_Repo+"/apps/"+pkg, Software_Repo+"/apps/"+pkg_prefix+pkg)
...
def upload_pkg_to_JSS(jss_share,jss_share_username, jss_share_password,jss_hostname,pkg_absolute_path):
	...
	if not os.path.exists("/Volumes/"+jss_share):
		...
		try:
			check_output(["mount_smbfs", "//"+jss_share_username+":"+jss_share_password+"@"+jss_hostname+"/"+jss_share,"/Volumes/"+jss_share])
	...
	call(["cp",pkg_absolute_path,jss_share])
...
def create_pol_xml(jss_category_id, jss_category_name, jss_pol_id, jss_pkg_id, pkg_pol_xml, pkg_full_name):
	...
	<policy>
	  <general>
	    ...
	    <trigger_other>{pkg_full_name}</trigger_other>
